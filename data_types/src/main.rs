...
fn main() {
    ...
    // ಠ 是 3232, 如果强制转 i8 的话, 高位会被截断, 变成 -96
    let k = '%' as i8; // 37
    let l = 'ಠ' as i8; // 96
    ...
    /* 类型转换 */
    // 在编程语言中, 类型转换分为隐式类型转换(Implicit Type Conversion)和显式类型转换(ExplicitType Conversion).
    // 隐式类型转换是由编译器或解释器来完成的, 开发者并未参与, 所以又称之为强制类型转换(Type Coercion).
    // 显式类型转换是由开发者指定的, 就是一般意义上的类型转换(Type Cast).
    //
    // 解引用: 分为自动解引用和手动解引用
    //
    // 自动解引用是编译器来做的, 但自动解引用的行为可以由开发者来定义, 举个例子:
    // 我们知道 Rust 的字符串相加, 右值应该是 &str 类型, 但下面显然是 &String.
    // 之所以不报错, 是因为 String 类型实现了 DeRef
    // ```
    ...
    // 其他的, &Vec<T> 可以转成 &[T]; &Rc<T> 可以转成 &T
    //
    // 手动解引用
    // 下面这个例子, clone 方法在 Re 和 &str 类型中都被实现, 因此要想是给 &str 使用 clone, 得手动把 x 解引用一次
    #[allow(unused)]
    let x = Rc::new("hello");
    #[allow(unused)]
    let y = x.clone();
    ...
    // as cast
    // 最常见的就是 as, 经常用来转换数字类型, 需要注意长类型转短类型可能被截断处理
    // 另外当从有符号类型向无符号类型转换的时候, 最好使用标准库中提供的专门的方法
    ...
    <S as A>::test(&s, 1);
    <S as B>::test(&s, 1);
    // 这两种方式叫作无歧义完全限定语法(Fully Qualified Syntax for Disambiguation)
    // 建议使用第二种方式, 因为<S as A>::test() 语义比较完整, 它表明了调用的是 S 结构体实现的 trait A 中的 test 方法

    // as 还可以进行类型和子类型相互转换
    // 生命周期标记可看作子类型, 比如 &'static str 类型是 &'a str 类型的子类型，
    // 因为二者的生命周期标记不同, 'a 和 'static 都是生命周期标记, 其中 'a 是泛型标记, 是 &str的通用形式，
    // 而 'static 则是特指静态生命周期的 &str 字符串. 所以, 通过 as 操作符转换可以将 &'static str 类型转为 &'a str 类型
